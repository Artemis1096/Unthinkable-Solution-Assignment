  ...
  - `tesseract.js` for Image OCR (Optical Character Recognition)

## Features
- **Drag-and-Drop Upload**: Easy file upload for PDFs and Images (.png, .jpg).
- **Text Extraction**: Automatically extracts text from uploaded files.
- **Engagement Suggestions**: Generates simple tips based on text length, hashtags, and engagement hooks (questions, CTAs).
- **History**: Saves analysis results to MongoDB.

## Architecture
- **`/server`**: Express API handling file uploads, text extraction, and database operations.
  - `src/models/Analysis.js`: Mongoose schema for storing results.
  - `src/controllers/analyzeController.js`: Core logic for parsing and analyzing text.
- **`/client`**: React application for the user interface.
  - `src/components/UploadArea.jsx`: Handles file selection and drag-and-drop.
  - `src/components/AnalysisResult.jsx`: Displays extracted text and suggestions.
...
```env
PORT=5000
MONGO_URI=mongodb://localhost:27017/social-media-analyzer
